...
//	private int executeUpdate;
	...
	public List<Board> getBoardlist(Pagination pagination, Search search) {
		...
		try {
			...
			if (search.getValue() == null || search.getValue() == "") { // 검색어가 없으면
				...
				String query = new StringBuilder().append("SELECT 		@ROWNUM := @ROWNUM - 1 AS ROWNUM,\n")
						.append("				ta.*\n").append("FROM 			board ta\n")
						.append("INNER JOIN	(SELECT @rownum := (SELECT	COUNT(*)-?+1 FROM board ta WHERE "
								+ search.getField() + " LIKE ?)) tb ON 1=1\n")
	...
	public void insertBoard(Board board) {
		...
		try {
			...
			sql = "select * from board last_insert_id";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public void insertBoardFile(Board board) {
		...
		try {
			...
			sql = "update boardfile set f_idx=last_insert_id() where f_idx=last_insert_id()";
			pstmt = conn.prepareStatement(sql);
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
